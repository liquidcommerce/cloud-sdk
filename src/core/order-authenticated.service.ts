...
export class OrderAuthenticatedService {
  ...
  public getUniqueKey(): string {
    return `${this.userID}_${this.password}_${this.baseURL}`;
  }

  /**
   * Authenticates the user with the LiquidCommerce Order API.
   *
   * This method sends a POST request to the /api/order-authentication endpoint,
  ...
  public async authenticate(): Promise<void> {
    ...
    try {
      ...
      console.error('Order Authentication failed:', error);
      throw new Error('Failed to authenticate with LiquidCommerce Order API');
    } finally {
  ...
  private async requestWithoutAuth<T>(path: string, options: IRequestOptions): Promise<T> {
    ...
    // Encode the userID and password using Base64
